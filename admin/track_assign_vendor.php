...
<tr><th> </th><th>Ven.order Id</th><th>Vendor dc reference</th> <th>Vendor dc date</th><th>Vendor invoice Number</th><th>Vendor invoice date</th><th>Material Receipt no</th><th>Payment Date</th><th>Received Qty</th><th>Received ON</th><th>Send by vendor Qty</th><th>Approved Qty</th><th>Rejection Qty</th> <th>Status</th><th>Vendor Payment</th><th>Remarks</th><th>Update</th></tr>
<?php 
...
$customer_proforma=$db->queryUniqueObject("SELECT * FROM customer_proforma WHERE proposed_item_qty_id='$inv[id]'"); 
 ?>
<tr id="vendorlist">


  <td><input type="checkbox" name="prof" value="<?=$inv[id]?>"  <?php if($customer_proforma->proposed_item_qty_id == $inv['id']) { ?> disabled <?php } ?>></td>
<td><?=$inv['vendor_order_id']; ?></td>
<td><?=$inv['vendor_dc_num']; ?><br /></td>
<td><?=$inv['vendor_dc_date']; ?><br /></td>
<td><?=$inv['invoice_num']; ?><br /></td>
<td><?=$inv['invoice_date']; ?><br /></td>
<td><?=$inv['payment_receipt']; ?><br /></td>
<td><?=$inv['payment_date']; ?><br /></td>
<td><?=$inv['sf_receive_qty']; ?><br /></td>
<td><?=$inv['sf_receive_time']; ?><br /></td>
<td><?=$inv['ven_send_qty']; ?></td>
<td><?=$inv['sf_approve_qty']; ?><br /></td>
<td><?=$inv['sf_reject_qty']; ?></td>
<td><?=$inv['status']; ?></td>
<td><?php $payment_type=$db->queryUniqueObject("select payment_type from vendor where id='$inv[ventor_id]'"); 
if($payment_type->payment_type == 'against credit'){ ?>
	<a href="credit_payment_order.php" class="btn btn-primary">Go to payment</a>
<?php } else if($payment_type->payment_type == 'against proforma'){
  if($inv['proforma_status'] =='received'){
	  
  	?>
	<a class="btn btn-primary" href="track_proforma.php?getid=<?=$inv['vendor_order_id'];?>&vid=<?=$inv['ventor_id']; ?>">Update Proforma</a>
<?php }else{ ?>
	<a href="proforma.php">Go to Request Proforma</a>
<?php }
 }
?>
<?php if($inv['proforma_status'] =='notreceived'){
...
<?php } ?><br /></td>
<td><?=$inv['remarks']; ?><br /></td>
<td>
<button class="btn btn-info btn-sm" data-toggle="modal" data-target=".editqty<?php echo $inv['id']; ?>"> <i class="fa fa-pencil"></i></button>
 <div class="modal fade editqty<?php echo $inv['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
                                                        <div class="modal-dialog modal-md">
                                                            <div class="modal-content">
															  <form name="payment" method="POST">
                                                                <div class="modal-header">
                                                                    <h5 class="modal-title mt-0" id="mySmallModalLabel">Tracking</h5>
...
<input type="hidden" name="payment_date" value="<?=$inv['payment_date']; ?>">
													...
													<div class="col-md-6">
													<label style="display:block;">Vendor DC Number</label>
													<input type="text" name="dc_no"  class="form-control tog<?=$i;?>" value="<?=$inv['vendor_dc_num']; ?>">
													</div>
													<div class="col-md-6">
													<label style="display:block;">Vendor DC Date</label>
													<input type="date" value="<?=$inv['vendor_dc_date'] ?>" name="dc_date" class="form-control tog<?=$i;?>" value="">
													</div>													
													<div class="col-md-6">
													<label style="display:block;">Invoice Number</label>
													<input type="text" name="invoice_no" class="form-control tog<?=$i;?>" value="<?=$inv['invoice_num']; ?>" required>
													</div>
													<div class="col-md-6">
													<label style="display:block;">Invoice Date</label>
													<input type="date" value="<?=$inv['vendor_dc_date'] ?>" name="invoice_date" class="form-control tog<?=$i;?>" value="">
													</div>
													<div class="col-md-6">
													<label style="display:block;">Receive Qty</label>
													<input type="text" name="receive_qty" class="form-control tog<?=$i;?>" value="<?=$inv['sf_receive_qty']; ?>">
													</div>
													
													<div class="col-md-6">
													<label style="display:block;">Receive Date</label>
													<input type="date" value="<?=$inv['sf_receive_time'] ?>" name="receive_date" class="form-control tog<?=$i;?>" value="">
													</div>
													<div class="col-md-6">
													<label style="display:block;">Approve Qty</label>
													<input type="number" name="approve_qty" max="<?=$inv['ven_send_qty']; ?>" class="form-control tog<?=$i;?>" value="<?=$inv['sf_approve_qty']; ?>">
													</div>
													<div class="col-md-6">
													<label style="display:block;">Proforma Status</label>
													<select name="proforma_status" class="form-control tog<?=$i;?>">
													...
													<input type="text" name="remarks" class="form-control tog<?=$i;?>" value="<?=$inv['remarks']; ?>">
													</div>
											</div>
                                                        </div>
                                                        <div class="modal-footer">
                                                            <button type="submit"  name="update_vendor_inv" class="btn btn-primary">Save</button>
...
<button type="button" id="generate" class="btn btn-primary"> Generate Proforma</a>
<script type="text/javascript">
    $(document).ready(function() {
        $("#generate").click(function(){
            var favorite = [];
            $.each($("input[name='prof']:checked"), function(){            
                favorite.push($(this).val());
            });
            //alert("My favourite sports are: " + favorite.join(", "));
			window.location = "customer_proforma.php?id=" + favorite.join(",") + "&corder=<?=$orderid; ?>";
        });
    });
</script>
