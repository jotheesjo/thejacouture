            ...
            <table width="100%"  class="table convert-data-table table-striped" id="datatable">
                ...
                <tbody>
                    ...
                    if(isset($_POST['search'])){
                        ...
                        $order = $db->query("select * from orders WHERE payment_status='success' AND date>='$from' AND date<='$to' ");

                    }else{
                        $order = $db->query("select * from orders WHERE payment_status='success' ORDER By date DESC ");
                    }
                    $sno = 0;
                    while($row1 = mysqli_fetch_array($order)){ $sno++;
                       ...
                       <tr role="row" class="odd <?php if($row1['notification']=='1'){ echo 'fresh'; } ?>">
                        ...
                        <td><a href="order-information.php?ordid=<?=$row1['id'];?>&userid=<?=$row1['user_id'];?>" class="btn btn-sm btn-primary"><i class="lnr lnr-eye"></i></a>
                           <a href="shipping.php?ordid=<?=$row1['id'];?>&userid=<?=$row1['user_id'];?>" class="btn btn-sm btn-warning"><i class="lnr lnr-exit"></i></a>
                           </td>
                    </tr>
